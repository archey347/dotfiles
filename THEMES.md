...
and notifications stay flat. Windows get a faint 1px outline, and the floating bar is
rounded like the windows. Pick an `accent_strong` that contrasts with `surface` so
resting keys read as raised.
